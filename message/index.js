...
module.exports = msgHandler = async (hisoka = new Client(), message) => {
    try {
        ...
        const botNumber = await hisoka.getHostNumber() + '@c.us'
        const blockNumber = await hisoka.getBlockedIds()
        const ownerNumber = config.ownerBot
        const groupId = isGroupMsg ? chat.groupMetadata.id : ''
        const groupAdmins = isGroupMsg ? await hisoka.getGroupAdmins(groupId) : ''
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isDetectorOn && !isOwner) {
            if (chats.match(new RegExp(/(https:\/\/chat.whatsapp.com)/gi))) {
                const valid = await hisoka.inviteInfo(chats)
                if (valid) {
                    console.log(color('[KICK]', 'red'), color('Received a group link and it is a valid link!', 'yellow'))
                    await hisoka.reply(from, eng.linkDetected(), id)
                    await hisoka.removeParticipant(groupId, sender.id)
                } else {
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isDetectorOn && !isOwner) {
            if (chats.match(new RegExp(/(https:\/\/chat.(?!whatsapp.com))/gi))) {
                ...
                await hisoka.reply(from, 'Fake group link detected!', id)
                await hisoka.removeParticipant(groupId, sender.id)
            }
        }
            ...
            if (isUrl(chats)) {
                ...
                isPorn(classify.hostname, async (err, status) => {
                    ...
                    if (status) {
                        ...
                        await hisoka.reply(from, eng.linkNsfw(), id)
                        await hisoka.removeParticipant(groupId, sender.id)
                    } else {
        ...
        if (isGroupMsg && isAutoStickerOn && isMedia && isImage && !isCmd) {
            ...
            await hisoka.sendImageAsSticker(from, imageBase64)
        ...
        if (isGroupMsg) {
            for (let ment of mentionedJidList) {
                if (afk.checkAfkUser(ment, _afk)) {
                    ...
                    await hisoka.reply(from, eng.afkMentioned(getReason, getTime), id)
                }
            }
            if (afk.checkAfkUser(sender.id, _afk) && !isCmd) {
                _afk.splice(afk.getAfkPosition(sender.id, _afk), 1)
                fs.writeFileSync('./database/user/afk.json', JSON.stringify(_afk))
                await hisoka.sendText(from, eng.afkDone(pushname))
            }
        }
        ...
        /*
        if (chats == 'p') {
            if (!isGroupMsg) await hisoka.reply(from, `Halo Kak, Untuk Memulai bot silahkan ketik ${prefix}menu`, id)
        }
        ...
        if (chats == 'assalamualaikum') {
            if (!isGroupMsg) await hisoka.reply(from, `Waalaikumsalam , Halo Kak, Untuk Memulai bot silahkan ketik ${prefix}menu`, id)
        }
        ...
        */
        ...
        switch (command) {
            case 'antiporn': // Premium, chat VideFikri
                await hisoka.reply(from, 'Premium Feature!\n\nContact: wa.me/6285692655520?text=Buy%20Anti%20Porn', id)
            break

            // Register by Slavyan
            case 'register':
                if (isRegistered) return await hisoka.reply(from, eng.registeredAlready(), id)
                if (isGroupMsg) return await hisoka.reply(from, eng.pcOnly(), id)
                if (!q.includes('|')) return await hisoka.reply(from, eng.wrongFormat(), id)
                const namaUser = q.substring(0, q.indexOf('|') - 1)
                const umurUser = q.substring(q.lastIndexOf('|') + 2)
                const serialUser = createSerial(20)
                if (Number(umurUser) >= 40) return await hisoka.reply(from, eng.ageOld(), id)
                register.addRegisteredUser(sender.id, namaUser, umurUser, time, serialUser, _registered)
                await hisoka.reply(from, eng.registered(namaUser, umurUser, sender.id, time, serialUser), id)
                console.log(color('[REGISTER]'), color(time, 'yellow'), 'Name:', color(namaUser, 'cyan'), 'Age:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'))
            break

            // Level [BETA] by Slavyan
            case 'level':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isLevelingOn) return await hisoka.reply(from, eng.levelingNotOn(), id)
                if (!isGroupMsg) return await hisoka.reply(from, eng.groupOnly(), id)
                const userLevel = level.getLevelingLevel(sender.id, _level)
                const userXp = level.getLevelingXp(sender.id, _level)
                const ppLink = await hisoka.getProfilePicFromServer(sender.id)
                if (ppLink === undefined) {
                    var pepe = errorImg
                } else {
                    pepe = ppLink
                ...
                rank.build()
                    .then(async (buffer) => {
                        ...
                        await hisoka.sendFile(from, `${sender.id}_card.png`, `${sender.id}_card.png`, '', id)
            ...
            case 'leaderboard':
                ...
                try {
                    ...
                    await hisoka.reply(from, leaderboard, id)
                } catch (err) {
                    console.error(err)
                    await hisoka.reply(from, eng.minimalDb(), id)
                }
            break
            ...
            case 'joox': // By Hafizh
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!q) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                ...
                cardJoox.build()
                    .then(async (buffer) => {
                        ...
                        await hisoka.sendFile(from, `${sender.id}_joox.png`, 'joox.png', eng.joox(dataJoox.data), id)
                        fs.unlinkSync(`${sender.id}_joox.png`)
                        await hisoka.sendFileFromUrl(from, dataJoox.data.result[0].linkMp3, 'joox.mp3', '', id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'instadl':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isUrl(url) && !url.includes('instagram.com')) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                    ...
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.post.length; i++) {
                            if (result.post[i].type === 'image') {
                                await hisoka.sendFileFromUrl(from, result.post[i].urlDownload, 'igpostdl.jpg', `*...:* *Instagram Downloader* *:...*\n\nUsername: ${result.owner_username}\nCaption: ${result.caption}`, id)
                            } else if (result.post[i].type === 'video') {
                                await hisoka.sendFileFromUrl(from, result.post[i].urlDownload, 'igpostdl.mp4', `*...:* *Instagram Downloader* *:...*\n\nUsername: ${result.owner_username}\nCaption: ${result.caption}`, id)
                            }
                        }
            ...
            case 'facebook':
            case 'fb':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(pushname), id)
                if (!isUrl(url) && !url.includes('facebook.com')) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                downloader.fb(url)
                    .then(async ({ result }) => {
                            await hisoka.sendFileFromUrl(from, result.VideoUrl, 'videofb.mp4', '', id)
            ...
            case 'ytmp3':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isUrl(url) && !url.includes('youtu.be')) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                downloader.ytdl(url)
                    .then(async (res) => {
                        if (res.status === 'error') {
                            await hisoka.reply(from, res.pesan, id)
                        } else if (Number(res.size.split(' MB')[0]) >= 30) {
                            await hisoka.reply(from, eng.videoLimit(), id)
                        } else {
                            await hisoka.sendFileFromUrl(from, res.thumbnail, `${res.title}.jpg`, eng.ytFound(res), id)
                            await hisoka.sendFileFromUrl(from, res.url_audio, `${res.title}.mp3`, '', id)
                ...
                downloader.ytdl(url)
                    .then(async (res) => {
                        if (res.status === 'error') {
                            ...
                            await hisoka.sendFileFromUrl(from, res.thumbnail, `${res.title}.jpg`, eng.ytFound(res), id)
                            await hisoka.sendFileFromUrl(from, res.url_video, `${res.title}.mp4`, '', id)
                            console.log('Success sending YouTube video!')
                        }
                    })
                    .catch(async (err) => {
            ...
            case 'tiktokpic':
                ...
                try {
                    ...
                    if (tkt.data.error) return hisoka.reply(from, tkt.data.error, id)
                    await hisoka.sendFileFromUrl(from, tkt.data.result, 'tiktokpic.jpg', 'Ini :D', id)
            ...
            case 'tktnowm':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isUrl(url) && !url.includes('tiktok.com')) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                downloader.tikNoWm(url)
                    .then(async ({ result }) => {
                        await hisoka.sendFileFromUrl(from, result.thumb, 'TiktokNoWM.jpg', `➸ *Username*: ${result.username}\n➸ *Caption*: ${result.caption}\n➸ *Uploaded on*: ${result.uploaded_on}\n\nSedang dikirim, sabar ya...`, id)
                        const responses = await fetch(result.link);
                        const buffer = await responses.buffer();
                        fs.writeFileSync(`./temp/${sender.id}_TikTokNoWm.mp4`, buffer)
                        await hisoka.sendFile(from, `./temp/${sender.id}_TikTokNoWm.mp4`, `${sender.id}_TikTokNoWm.mp4`, '', id)
            ...
            case 'tiktok':
                ...
                downloader.tik(url)
                    .then(async ({ result })=> {
                        ...
                        await hisoka.sendFile(from, `./temp/${sender.id}_TikTok.mp4`, `${sender.id}_TikTok.mp4`, '', id)
            ...
            case 'twt':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isUrl(url) && !url.includes('twitter.com')) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                downloader.tweet(url)
                    .then(async (data) => {
                        if (data.type === 'video') {
                            ...
                            await hisoka.sendFileFromUrl(from, content[0].url, 'video.mp4', `Link HD: ${result}`, id)
                            ...
                            for (let i = 0; i < data.variants.length; i++) {
                                await hisoka.sendFileFromUrl(from, data.variants[i], data.variants[i].split('/media/')[1], '', id)
                                .then(() => console.log('Success sending Twitter media!'))
                                .catch(async (err) => {
                                    console.error(err)
                                    await hisoka.reply(from, 'Error!', id)
            ...
            case 'moddroid': // Chikaa Chantekkzz
                ...
                downloader.modroid(q)
                    .then(async ({ status, result }) => {
                        if (status !== 200) {
                            await hisoka.reply(from, 'Not found.', id)
                        } else {
                            await hisoka.sendFileFromUrl(from, result[0].image, 'ksk.jpg', `*「 MOD FOUND 」*\n\n➸ *APK*: ${result[0].title}\n\n➸ *Size*: ${result[0].size}\n➸ *Publisher*: ${result[0].publisher}\n➸ *Version*: ${result[0].latest_version}\n➸ *Genre*: ${result[0].genre}\n\n*Download link*\n${result[0].download}`, id)
            ...
            case 'linedl': // chikaa chantexxzz
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (isGroupMsg) return await hisoka.reply(from, eng.pcOnly(), id)
                if (!isUrl(url) && !url.includes('store.line.me')) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                downloader.line(url)
                    .then(async (res) => {
                        await hisoka.sendFileFromUrl(from, res.thumb, 'line.png', `*「 LINE STICKER DOWNLOADER 」*\n\n➸ *Title*: ${res.title}\n➸ *Sticker type*: ${res.type}\n\n_Media sedang dikirim, mohon tunggu sebentar..._`, id)
                        for (let i = 0; i < res.sticker.length; i++) {
                            await hisoka.sendStickerfromUrl(from, `${res.sticker[i]}`)
            ...
            case 'googlesearch':
                ...
                google({ 'query': q, 'no-display': true })
                    .then(async (results) => {
                        ...
                        await hisoka.reply(from, txt, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'say':
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.sendText(from, q)
            break
            case 'afk': // by Slavyan
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isGroupMsg) return await hisoka.reply(from, eng.groupOnly(), id)
                if (isAfkOn) return await hisoka.reply(from, eng.afkOnAlready(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const reason = q ? q : 'Nothing.'
                afk.addAfkUser(sender.id, time, reason, _afk)
                await hisoka.reply(from, eng.afkOn(pushname, reason), id)
            break
            case 'lyric':
            case 'lirik':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                misc.lirik(q)
                    .then(async ({ result }) => {
                        if (result.code !== 200) return await hisoka.reply(from, 'Not found.', id)
                        await hisoka.reply(from, result.result, id)
            ...
            case 'shortlink':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isUrl(url)) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const urlShort = await misc.shortener(url)
                await hisoka.reply(from, eng.wait(), id)
                await hisoka.reply(from, urlShort, id)
                console.log('Success!')
            break
            case 'wikipedia':
            case 'wiki':
                ...
                misc.wiki(q)
                    .then(async ({ result, status }) => {
                        if (status !== 200) {
                            return await hisoka.reply(from, 'Not found.', id)
                        } else {
                            await hisoka.reply(from, result, id)
                ...
                misc.wikien(q)
                    .then(async ( { result }) => {
                        if (result.status !== '200') {
                            ...
                            await hisoka.reply(from, `➸ *PageId*: ${result.pageid}\n➸ *Title*: ${result.title}\n➸ *Result*: ${result.desc}`, id)
                        }
                    })
                    .catch(async (err) => {
                        console.error(err)
            ...
            case 'ttp': // CHIKAA CHANTEKKXXZZ
                ...
                misc.ttp(q)
                    .then(async (res) => {
                        await hisoka.sendImageAsSticker(from, res.base64)
            ...
            case 'genshin':
                ...
                try {
                    ...
                    await hisoka.sendFileFromUrl(from, character.image, `${character.title}.jpg`, `*「 GENSHIN IMPACT 」*\n\n*${character.title}*\n${character.description}\n➸ *Name*: ${character.name}\n➸ *Nation*: ${character.nation}\n➸ *Gender*: ${character.gender}\n➸ *Birthday*: ${character.birthday}\n➸ *Constellation*: ${character.constellation}\n➸ *Rarity*: ${character.rarity}\n➸ *Vision*: ${character.vision}\n➸ *Weapon*: ${character.weapon}\n\n${character.url}`)
                    console.log('Success sending Genshin Impact character!')
                } catch (err) {
                    console.error(err)
                    await hisoka.reply(from, 'Error or character not found!', id)
                }
            break
            case 'instastory': // By: VideFrelan
            case 'igstory':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!q) return hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                misc.its(q)
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.story.itemlist.length; i++) {
                            ...
                            await hisoka.sendFileFromUrl(from, urlDownload, '', 'By: VideFrelan', id)
            ...
            case 'kbbi':
                ...
                misc.kbbi(q)
                    .then(async ({ result }) => {
                        await hisoka.reply(from, result.hasil, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'linestiker':
                ...
                misc.linesticker()
                    .then(async ({ result }) => {
                        ...
                        await hisoka.reply(from, lines, id)
                ...
                misc.jadwalSholat(q)
                    .then((data) => {
                        data.map(async ({isya, subuh, dzuhur, ashar, maghrib, terbit}) => {
                            ...
                            await hisoka.reply(from, msg, id)
            ...
            case 'gempa':
                ...
                misc.bmkg()
                    .then(async ({ kedalaman, koordinat, lokasi, magnitude, map, potensi, waktu }) => {
                        ...
                        await hisoka.sendFileFromUrl(from, map, 'gempa.jpg', teksInfo, id)
                ...
                misc.igStalk(q)
                    .then(async ({ graphql }) => {
                        if (graphql === undefined) {
                            ...
                            await hisoka.sendFileFromUrl(from, profile_pic_url_hd, 'insta.jpg', text, id)
            ...
            case 'gsmarena':
                ...
                try {
                    misc.gsmarena(q)
                        .then(async ({ result }) => {
                            await hisoka.sendFileFromUrl(from, result.image, `${result.title}.jpg`, eng.gsm(result), id)
            ...
            case 'resep':
                ...
                try {
                    misc.resep(q)
                        .then(async ({ result }) => {
                            await hisoka.sendFileFromUrl(from, result.image, `${result.title}.jpg`, eng.receipt(result), id)
            ...
            case 'findstiker':
                ...
                try {
                    misc.sticker(q)
                        .then(async ({ result }) => {
                            if (result.response !== 200) return await hisoka.reply(from, 'Not found!', id)
                            for (let i = 0; i < result.data.length; i++) {
                                await hisoka.sendStickerfromUrl(from, result.data[i])
                            }
                            console.log('Success sending sticker!')
                        })
                } catch (err) {
                    console.error(err)
                    await hisoka.reply(from, `Error!\n\n${err}`, id)
                }
            break
            case 'movie':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                misc.movie(q)
                    .then(async ({ result }) => {
                        ...
                        movies += '\n\nBy: VideFrelan'
                        await hisoka.reply(from, movies, id)
            ...
            case 'cekongkir': // By: VideFrelan
                ...
                misc.ongkir(kurir, askot, tukot)
                    .then(async ({ result }) => {
                        ...
                        onkir += '\n\nBy: VideFrelan'
                        await hisoka.reply(from, onkir, id)
                ...
                misc.distance(kotaAsal, kotaTujuan)
                    .then(async ({ result }) => {
                        if (result.response !== 200) {
                            ...
                            await hisoka.reply(from, result.data, id)
                    ...
                    misc.ytSearch(q)
                        .then(async ({ result }) => {
                            for (let i = 0; i < 5; i++) {
                                ...
                                await hisoka.sendFileFromUrl(from, image, `${title}.jpg`, eng.ytResult(urlyt, title, channel, duration, views), id)
            ...
            case 'tts':
                ...
                try {
                    ptt.save(`${speech}.mp3`, speech, async () => {
                        await hisoka.sendPtt(from, `${speech}.mp3`, id)
            ...
            case 'tomp3': // by: Piyobot
                ...
                if ((isMedia && isVideo || isQuotedVideo)) {
                    ...
                    fs.writeFile(fileInputPath, mediaData, (err) => {
                        ...
                        ffmpeg(fileInputPath)
                            ...
                            .on('end', async () => {
                                ...
                                await hisoka.sendFile(from, fileOutputPath, 'audio.mp3', '', id)
                    ...
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'playstore':
            case 'ps':
                    ...
                    misc.playstore(q)
                        .then(async ({ result }) => {
                            for (let i = 0; i < 5; i++) {
                                ...
                                await hisoka.sendFileFromUrl(from, icon, `${title}.jpg`, eng.playstore(app_id, title, developer, description, price, free))
                            }
            ...
            case 'math':
                ...
                if (typeof mathjs.evaluate(q) !== 'number') {
                    await hisoka.reply(from, eng.notNum(q), id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.reply(from, `*「 MATH 」*\n\n${q} = ${mathjs.evaluate(q)}`, id)
                }
            break
            case 'shopee':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                    ...
                    misc.shopee(namaBarang, jumlahBarang)
                        .then(async ({ result }) => {
                            for (let i = 0; i < result.items.length; i++) {
                                ...
                                await hisoka.sendFileFromUrl(from, image_cover, `${nama}.jpg`, eng.shopee(nama, harga, terjual, shop_location, description, link_product))
                            }
            ...
            case 'mutual':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (isGroupMsg) return await hisoka.reply(from, 'Command ini tidak bisa digunakan di dalam grup!', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, 'Looking for a partner...', id)
                await hisoka.sendContact(from, register.getRegisteredRandomId(_registered))
                await hisoka.sendText(from, `Partner found: 🙉\n*${prefix}next* — find a new partner`)
            break
            case 'next':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (isGroupMsg) return await hisoka.reply(from, 'Command ini tidak bisa digunakan di dalam grup!', id)
            ...
            case 'tafsir':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (args.length === 0) return hisoka.reply(from, `Untuk menampilkan ayat Al-Qur'an tertentu beserta tafsir dan terjemahannya\ngunakan ${prefix}tafsir surah ayat\n\nContoh: ${prefix}tafsir Al-Mulk 10`, id)
                await hisoka.reply(from, eng.wait(), id)
                ...
                if (!isNaN(nomerSurah)) {
                    ...
                    await hisoka.reply(from, pesan, id)
                }
            break
            case 'listsurah':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                misc.listSurah()
                    .then(async ({ result }) => {
                        ...
                        await hisoka.reply(from, list, id)
            ...
            case 'surah':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (args.length !== 1) return await hisoka.reply(from, eng.wrongFormat(), id)
                await hisoka.reply(from, eng.wait(), id)
                misc.getSurah(args[0])
                    .then(async ({ result }) => {
                        await hisoka.reply(from, `${result.surah}\n\n${result.quran}`, id)
            ...
            case 'asmaulhusna': // irham01
                ...
                hisoka.sendFileFromUrl(from, 'https://i2.wp.com/seruni.id/wp-content/uploads/2016/09/Allah.png?resize=696%2C696&ssl=1', 'gambar.jpg', eng.asmaulHusna(assna), id)
            break
            case 'randomquran': // irham01
                ...
                await hisoka.reply(from, eng.randomQuran(ranquran), id)
                await hisoka.sendFileFromUrl(from, auquran, 'rquran.mp3', '', id)
            break
            case 'motivasi':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                ...
                misc.motivasi()
                    .then(async (body) => {
                        ...
                        await hisoka.sendText(from, randomMotivasi)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'play':
                ...
                misc.ytPlay(q)
                    .then(async ({ result }) => {
                        if (Number(result.size.split(' MB')[0]) >= 10.0) return hisoka.sendFileFromUrl(from, result.image, `${result.title}.jpg`, `Judul: ${result.title}\nSize: *${result.size}*\n\nGagal, Maksimal video size adalah *10MB*!`, id)
                        await hisoka.sendFileFromUrl(from, result.image, `${result.title}.jpg`, eng.ytPlay(result), id)
                        const responses = await fetch(result.mp3);
                        const buffer = await responses.buffer(); 
                        await fs.writeFile(`./temp/${result.title}.mp3`, buffer)
                        await hisoka.sendFile(from, `./temp/${result.title}.mp3`, `${result.title}`, id)
            ...
            case 'playv': // Alvio Adji Januar 
                ...
                if (getvid.data.status === false) {
                    await hisoka.reply(from, getvid.data.message, id)
                } else if (Number(getvid.data.result.size.split('MB')[0]) >= 10.00) {
                    await hisoka.reply(from, 'Maaf durasi musik sudah melebihi batas maksimal 10 MB!', id)
                } else {
                    await hisoka.sendFileFromUrl(from, getvid.data.result.thumb, 'thumb.jpg', `Title: ${getvid.data.result.title}\n\n───────────⚪───────────\n(っ◔◡◔)っ\n───────────⚪───────────\n➥Size: ${getvid.data.result.size}\n➥Type: Mp4\n➥Link Download: ${getvid.data.result.link}\n\n*Mohon Tunggu Bot Akan Mengirim Video!*`, id)
                    await hisoka.sendFileFromUrl(from, getvid.data.result.link, 'play.mp4' , '', id)
                }
            break
            case 'whois':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                misc.email(emailTarget, subjectEmail, messageEmail)
                    .then(async ({ result }) => {
                        if (result.status === '204') {
                            await hisoka.reply(from, 'Server busy!', id)
                        } else {
                            await hisoka.reply(from, `*Success sending email*!\n➸ *Target*: ${emailTarget}\n➸ *Subject*: ${result.subjek}\n➸ *Message*: ${result.pesan}`, id)
            ...
            case 'sms':
                ...
                misc.sms(nomorPenerima, pesanPengirim)
                    .then(async ({ status, pesan }) => {
                        if (status !== 'success') return await hisoka.reply(from, pesan, id)
                        await hisoka.reply(from, `Success sending SMS to: ${nomorPenerima}\nMessage: ${pesanPengirim}`, id)
            ...
            case 'toxic':
                if (!isRegistered) return await hisoka.reply(from , eng.notRegistered(), id)
                await hisoka.reply(from, toxic(), id)
            break
            case 'alkitab':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!q) return await hisoka.reply(from, eng.wrongFormat(), id)
                ...
                misc.alkitab(q)
                    .then(async ({ result }) => {
                        ...
                        await hisoka.reply(from, alkitab, id)
            ...
            case 'imgtourl':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await hisoka.reply(from, linkImg, id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'infohoax':
                ...
                misc.infoHoax()
                    .then(async ({ result }) => {
                        ...
                        await hisoka.sendFileFromUrl(from, result[0].image, 'hoax.jpg', txt, id)
                        console.log('Success sending info!')
                    })
                    .catch(async (err) => {
                        console.error(err)
            ...
            case 'spamcall':
                ...
                misc.call(q)
                    .then(async ({ result }) => {
                        await hisoka.reply(from, result.logs, id)
            ...
            case 'spamsms':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (args.length !== 2) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (isNaN(Number(args[0])) && isNaN(Number(args[1]))) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (Number(args[1]) > 10) return await hisoka.reply(from, 'Maximum 10 SMS.', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                misc.spamsms(args[0], args[1])
                    .then(async ({ status, logs, msg }) => {
                        if (status !== 200) {
                            ...
                            await hisoka.reply(from, logs, id)
            ...
            case 'trans':
                ...
                translate(texto, {to: languaget}).then(res => {hisoka.reply(from, res.text, id)})
            break
            ...
            case 'help':
                ...
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (args[0] === '1') {
                    await hisoka.sendText(from, eng.menuDownloader())
                } else if (args[0] === '2') {
                    await hisoka.sendText(from, eng.menuBot())
                } else if (args[0] === '3') {
                    await hisoka.sendText(from, eng.menuMisc())
                } else if (args[0] === '4') {
                    await hisoka.sendText(from, eng.menuSticker())
                } else if (args[0] === '5') {
                    await hisoka.sendText(from, eng.menuWeeaboo())
                } else if (args[0] === '6') {
                    await hisoka.sendText(from, eng.menuFun())
                } else if (args[0] === '7') {
                    await hisoka.sendText(from, eng.menuModeration())
                } else if (args[0] === '8') {
                    if (isGroupMsg && !isNsfw) return await hisoka.reply(from, eng.notNsfw(), id)
                    await hisoka.sendText(from, eng.menuNsfw())
                } else if (args[0] === '9') {
                    if (!isOwner) return await hisoka.reply(from, eng.ownerOnly())
                    await hisoka.sendText(from, eng.menuOwner())
                } else if (args[0] === '10') {
                    if (!isGroupMsg) return await hisoka.reply(from, eng.groupOnly(), id)
                    await hisoka.sendText(from, eng.menuLeveling())
                } else {
                    await hisoka.sendText(from, eng.menu(jumlahUser, levelMenu, xpMenu, role, pushname, reqXpMenu, isPremium ? 'YES' : 'NO'))
                }
            break
            case 'rules':
            case 'rule':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                await hisoka.sendText(from, eng.rules())
            break
            case 'nsfw':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isGroupMsg) return await hisoka.reply(from, eng.groupOnly(), id)
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isNsfw) return await hisoka.reply(from, eng.nsfwAlready(), id)
                    _nsfw.push(groupId)
                    fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(_nsfw))
                    await hisoka.reply(from, eng.nsfwOn(), id)
                } else if (ar[0] === 'disable') {
                    _nsfw.splice(groupId, 1)
                    fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(_nsfw))
                    await hisoka.reply(from, eng.nsfwOff(), id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'status':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                await hisoka.sendText(from, `*RAM*: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB / ${Math.round(os.totalmem / 1024 / 1024)} MB\n*CPU*: ${os.cpus()[0].model}`)
            break
            case 'listblock':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                await hisoka.sendTextWithMentions(from, block)
            break
            case 'ownerbot':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                await hisoka.sendContact(from, ownerNumber)
            break
            case 'runtime': // BY HAFIZH
                ...
                await hisoka.reply(from, `── *「 BOT UPTIME 」* ──\n\n ❏${formater(uptime)}`, id)
            break
            case 'ping':
            case 'p':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                await hisoka.sendText(from, `Pong!\nSpeed: ${processTime(t, moment())} secs`)
            break
            case 'delete':
            case 'del':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!quotedMsg) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (!quotedMsgObj.fromMe) return await hisoka.reply(from, eng.wrongFormat(), id)
                await hisoka.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
            case 'report':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!q) return await hisoka.reply(from, eng.emptyMess(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                ...
                if (lastReport !== undefined && cd - (Date.now() - lastReport) > 0) {
                    ...
                    await hisoka.reply(from, eng.daily(time), id)
                } else {
                    if (isGroupMsg) {
                        await hisoka.sendText(ownerNumber, `-----[ REPORT ]-----\n\n*From*: ${pushname}\n*ID*: ${sender.id}\n*Group*: ${(name || formattedTitle)}\n*Message*: ${q}`)
                        await hisoka.reply(from, eng.received(pushname), id)
                    } else {
                        await hisoka.sendText(ownerNumber, `-----[ REPORT ]-----\n\n*From*: ${pushname}\n*ID*: ${sender.id}\n*Message*: ${q}`)
                        await hisoka.reply(from, eng.received(pushname), id)
                    }
            ...
            case 'tos':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                await hisoka.sendLinkWithAutoPreview(from, 'https://github.com/dxxoo/HisokaBOT-Whatsapp-Bot', eng.tos(ownerNumber))
            break
            case 'join':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isUrl(url) && !url.includes('chat.whatsapp.com')) return await hisoka.reply(from, eng.wrongFormat(), id)
                const checkInvite = await hisoka.inviteInfo(url)
                if (isOwner) {
                    await hisoka.joinGroupViaLink(url)
                    await hisoka.reply(from, eng.ok(), id)
                    await hisoka.sendText(checkInvite.id, `Hello!! I was invited by ${pushname}`)
                } else {
                    const getGroupData = await hisoka.getAllGroups()
                    if (getGroupData.length >= groupLimit) {
                        await hisoka.reply(from, `Invite refused. Max group is: ${groupLimit}`, id)
                    } else if (getGroupData.size <= memberLimit) {
                        await hisoka.reply(from, `Invite refused. Minimum member is: ${memberLimit}`, id)
                    } else {
                        if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                        limit.addLimit(sender.id, _limit, isPremium, isOwner)
                        await hisoka.joinGroupViaLink(url)
            ...
            case 'cekpremium':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isPremium) return await hisoka.reply(from, eng.notPremium(), id)
                const cekExp = ms(premium.getPremiumExpired(sender.id, _premium) - Date.now())
                await hisoka.reply(from, `*「 PREMIUM EXPIRE 」*\n\n➸ *ID*: ${sender.id}\n➸ *Premium left*: ${cekExp.days} day(s) ${cekExp.hours} hour(s) ${cekExp.minutes} minute(s)`, id)
            break
            case 'premiumlist':
            case 'listpremium':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                for (let i = 0; i < deret.length; i++) {
                    ...
                    arrayPremi.push(await hisoka.getContact(premium.getAllPremiumUser(_premium)[i]))
                    listPremi += `${i + 1}. wa.me/${premium.getAllPremiumUser(_premium)[i].replace('@c.us', '')}\n➸ *Name*: ${arrayPremi[i].pushname}\n➸ *Expired*: ${checkExp.days} day(s) ${checkExp.hours} hour(s) ${checkExp.minutes} minute(s)\n\n`
                }
                await hisoka.reply(from, listPremi, id)
            break
            case 'getpic':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (mentionedJidList.length !== 0) {
                    const userPic = await hisoka.getProfilePicFromServer(mentionedJidList[0])
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    ...
                    await hisoka.sendFileFromUrl(from, pek, 'pic.jpg', '', id)
                } else if (args.length !== 0) {
                    const userPic = await hisoka.getProfilePicFromServer(args[0] + '@c.us')
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    if (userPic === undefined) {
                        var peks = errorImg
                    ...
                    await hisoka.sendFileFromUrl(from, peks, 'pic.jpg', '', id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'serial':
                if (!isRegistered) return await hisoka.reply(from, eng.registered(), id)
                if (isGroupMsg) return await hisoka.reply(from, eng.pcOnly(), id)
                if (args.length !== 1) return await hisoka.reply(from, eng.wrongFormat(), id)
                ...
                if (register.checkRegisteredUserFromSerial(serials, _registered)) {
                    ...
                    await hisoka.sendText(from, eng.registeredFound(name, age, time, serials, id))
                } else {
                    await hisoka.sendText(from, eng.registeredNotFound(serials))
                }
            break
            case 'limit':
                if (isPremium || isOwner) return await hisoka.reply(from, '⤞ Limit left: ∞ (UNLIMITED)', id)
                await hisoka.reply(from, `⤞ Limit left: ${limit.getLimit(sender.id, _limit, limitCount)} / 25\n\n*_Limit direset pada pukul 00:00 WIB_*`, id)
            break
            ...
            case 'kelpersegi':
                ...
                await hisoka.reply(from, `*Hasil*: ${persegi}\n*Rumus*: ${caraPersegi}`, id)
            break
            case 'luaspersegi':
                if (!isRegistered) return await hisoka.reply(from, eng.registered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                ...
                await hisoka.reply(from, `*Hasil*: ${luaspersegi}\n*Rumus*: ${luaspersegis}`, id)
            break
            case 'kuadrat':
                if (!isRegistered) return await hisoka.reply(from, eng.registered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                ...
                await hisoka.reply(from, `*Hasil*: ${kuadrat}`, id)
            break
            case 'kubik':
                if (!isRegistered) return await hisoka.reply(from, eng.registered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                ...
                await hisoka.reply(from, `*Hasil*: ${kubik}`, id)
            break
            ...
            case 'neko':
                ...
                await hisoka.sendFileFromUrl(from, (await neko.sfw.neko()).url, 'neko.jpg', '', null, null, true)
            ...
            case 'wp':
                ...
                await hisoka.sendFileFromUrl(from, (await neko.sfw.wallpaper()).url, 'wallpaper.jpg', '', null, null, true)
            ...
            case 'kemono':
                ...
                await hisoka.sendFileFromUrl(from, (await neko.sfw.kemonomimi()).url, 'kemono.jpg', '', null, null, true)
            ...
            case 'kusonime':
                ...
                weeaboo.anime(q)
                    .then(async ({ info, link_dl, sinopsis, thumb, title, error, status }) => {
                        if (status === false) {
                            return await hisoka.reply(from, error, id)
                        } else {
                            let animek = `${title}\n\n${info}\n\nSinopsis: ${sinopsis}\n\nLink download:\n${link_dl}`
                            await hisoka.sendFileFromUrl(from, thumb, 'animek.jpg', animek, null, null, true)
            ...
            case 'komiku':
                ...
                weeaboo.manga(q)
                    .then(async ({ genre, info, link_dl, sinopsis, thumb }) => {
                        ...
                        await hisoka.sendFileFromUrl(from, thumb, 'mangak.jpg', mangak, null, null, true)
                    ...
                    weeaboo.wait(imageBase64)
                        .then(async (result) => {
                            if (result.docs && result.docs.length <= 0) {
                                return await hisoka.reply(from, 'Anime not found! :(', id)
                            } else {
                                ...
                                if (similarity < 0.92) {
                                    ...
                                    await hisoka.sendFileFromUrl(from, video, `${title_romaji}.mp4`, teks, id)
                    ...
                    try {
                        ...
                        for (let i = 0; i < results.length; i++) {
                            ...
                            if (results[i].similarity < 80.00) {
                                ...
                                await hisoka.sendLinkWithAutoPreview(from, results[i].url, teks)
            ...
            case 'waifu':
                ...
                weeaboo.waifu(false)
                    .then(async ({ url }) => {
                        await hisoka.sendFileFromUrl(from, url, 'waifu.png', '', id)
            ...
            case 'anitoki':
                ...
                weeaboo.anitoki()
                    .then(async ({ result }) => {
                        ...
                        await hisoka.reply(from, anitoki, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'neonime':
                ...
                weeaboo.neonime()
                    .then(async ({ status, result }) => {
                        if (status !== 200) return await hisoka.reply(from, 'Not found.', id)
                        let neoInfo = '-----[ *NEONIME LATEST* ]-----'
                        for (let i = 0; i < result.length; i++) {
                            const { date, title, link, desc } = result[i]
                            neoInfo += `\n\n➸ *Title*: ${title}\n➸ *Date*: ${date}\n➸ *Synopsis*: ${desc}\n➸ *Link*: ${link}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        await hisoka.reply(from, neoInfo, id)
            ...
            case 'anoboy':
                ...
                weeaboo.anoboy()
                    .then(async ({ result }) => {
                        ...
                        await hisoka.reply(from, anoboyInfo, id)
            ...
            case 'animesticker': 
                ...
                weeaboo.snime()
                    .then(async (body) => {
                        ...
                        await hisoka.sendStickerfromUrl(from, wifegerakx)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'bapak': // By Kris
                ...
                axios.get(`https://api.terhambar.com/bpk?kata=${q}`)
                    .then(async (res) => await hisoka.reply(from, res.data.text, id))
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
                    })
            ...
            case 'puisi': // By Kris
                ...
                axios.get('https://masgi.herokuapp.com/api/puisi2')
                    .then(async (res) => await hisoka.reply(from, res.data.data, id))
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
                    })
            ...
            case 'creepyfact': // By Kris
                ...
                fetch('https://raw.githubusercontent.com/TheSploit/CreepyFact/main/creepy.txt')
                    ...
                    .then(async (body) => {
                        ...
                        await hisoka.reply(from, creepyz, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'quotes':
                ...
                misc.quotes()
                .then(async ({ result }) => {
                    await hisoka.reply(from, `➸ *Quotes*: ${result.quotes}\n➸ *Author*: ${result.author}`, id)
                })
            break
            case 'asupan': // shansekai
                ...
                fun.asupan()
                    .then(async (body) => {
                        ...
                        await hisoka.sendFileFromUrl(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, 'asupan.mp4', 'Follow IG: https://www.instagram.com/ptl_repost untuk mendapatkan asupan lebih banyak.', id)
            ...
            case 'citacita': // Piyobot
                ...
                fun.cita()
                    .then(async (body) => {
                        ...
                        await hisoka.sendFileFromUrl(from, randomCita, 'cita.mp3', '', id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'dadu': // by CHIKAA CHANTEKKXXZZ
                ...
                fun.dadu()
                    .then(async (body) => {
                        ...
                        await hisoka.sendStickerfromUrl(from, dadugerakx)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'doge':
                ...
                fun.doge()
                    .then(async (body) => {
                        ...
                        await hisoka.sendStickerfromUrl(from, dogegx)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'me':
                ...
                if (quotedMsg) {
                    ...
                    const profilePic = await hisoka.getProfilePicFromServer(getQuoted)
                    const username = quotedMsgObj.sender.name
                    const statuses = await hisoka.getStatus(getQuoted)
                    ...
                    await hisoka.sendFileFromUrl(from, pfp, `${username}.jpg`, eng.profile(username, status, premi, benet, adm, levelMe, req, xpMe), id)
                } else {
                    const profilePic = await hisoka.getProfilePicFromServer(sender.id)
                    const username = pushname
                    const statuses = await hisoka.getStatus(sender.id)
                    ...
                    await hisoka.sendFileFromUrl(from, pfps, `${username}.jpg`, eng.profile(username, status, premi, benet, adm, levelMe, req, xpMe), id)
                }
            break
            case 'hartatahta':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/hartatahta?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
            ...
            case 'nulis':
                ...
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/write?text=${q}&apikey=${config.vhtear}`, 'nulis.jpg', '', id)
            ...
            case 'ffbanner': // By: VideFrelan
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!q.includes('|')) return hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                ...
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/bannerff?title=${teks1ffanjg}&text=${teks2ffanjg}&apikey=${config.vhtear}`, id)
                console.log('Success!')
            break
            case 'caklontong': //By: VideFrelan
                if (!isGroupMsg) return hisoka.reply(from, eng.groupOnly(), id)
                if (!isRegistered) return  hisoka.reply(from, eng.notRegistered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                const sleep = (ms) => {
                ...
                fun.caklontong()
                    .then(async ( { result }) => {
                        await hisoka.reply(from, `➸ *Soal*: ${result.soal}`, id)
                        await hisoka.sendText(from, '30 Detik Tersisa...')
                        await sleep(10000)
                        await hisoka.sendText(from, '20 Detik Tersisa...')
                        await sleep(10000)
                        await hisoka.sendText(from, '10 Detik Tersisa...')
                        await sleep(10000)
                        await hisoka.reply(from, `➸ *Jawaban*: ${result.jawaban}\n${result.desk}`, id)
            ...
            case 'tebakgambar':
                ...
                fun.tbkgmbr()
                    .then(async ({ result }) => {
                        await hisoka.sendFileFromUrl(from, result.soal_gbr, 'TebakGambar.jpg', '', id)
                        await hisoka.sendText(from, '50 Detik Tersisa...')
                        await sleep(10000)
                        await hisoka.sendText(from, '40 Detik Tersisa...')
                        await sleep(10000)
                        await hisoka.sendText(from, '30 Detik Tersisa...')
                        await sleep(10000)
                        await hisoka.sendText(from, '20 Detik Tersisa...')
                        ...
                        await hisoka.sendText(from, '10 Detik Tersisa...')
                        await sleep(10000)
                        await hisoka.reply(from, `➸ *Jawaban*: ${result.jawaban}`, id)
            ...
            case 'fflogo': // By: VideFrelan
                ...
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=${karakter}&text=${teksff}&apikey=${config.vhtear}`, id)
                console.log('Success!')
            break
            case 'text3d':
            case '3dtext':
                ...
                await hisoka.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/text3d?text=${q}`,`${q}.jpg`, '', id)
            ...
            case 'simi': // by: VideFrelan
                ...
                fun.simi(q)
                    .then(async ({ jawaban }) => {
                        await hisoka.reply(from, jawaban, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, `Error!\n\n${err}`, id)
            ...
            case 'glitext':
                ...
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/glitchtext?text1=${teks1}&text2=${teks2}&apikey=${config.vhtear}`, 'glitch.jpg', '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'phmaker':
                ...
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${kiri}&text2=${kanan}&apikey=${config.vhtear}`, 'ph.jpg', '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'blackpink':
                ...
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/blackpinkicon?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
            ...
            case 'galaxy':
                ...
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/galaxytext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'tod':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                await hisoka.reply(from, 'Sebelum bermain berjanjilah akan melaksanakan apapun perintah yang diberikan.' , id)
                await hisoka.sendText(from, `Silakan ketik *${prefix}truth* atau *${prefix}dare*`)
            break
            case 'weton':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!q.includes('|')) return await hisoka.reply(from, eng.wrongFormat(), id)
            ...
            case 'truth':
                ...
                fun.truth()
                    .then(async (body) => {
                        ...
                        await hisoka.reply(from, randomTod, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
            ...
            case 'hilih':
                ...
                fun.hilihteks(q)
                    .then(async ( { result }) => {
                        await hisoka.reply(from, result.kata, id)
            ...
            case 'dare':
                ...
                fun.dare()
                    .then(async (body) => {
                        ...
                        await hisoka.reply(from, randomDare, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
                    ...
                    canvas.Canvas.trigger(mediaData)
                        .then((buffer) => {
                            ...
                            ffmpeg(fileInputPath)
                                ...
                                .on('end', async () => {
                                    console.log(color('[FFmpeg]', 'green'), 'Processing finished!')
                                    await hisoka.sendMp4AsSticker(from, fileOutputPath, { fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', loop: 0 })
            ...
            case 'trash':
                ...
                try {
                    ...
                    for (let i = 0; i < mentionedJidList.length; i++) {
                        const ypics = await hisoka.getProfilePicFromServer(mentionedJidList[i])
                        if (ypics === undefined) {
                            var ypfps = errorImg
                        } else {
                            ypfps = ypics
                    ...
                    canvas.Canvas.trash(ypfps)
                        .then(async (buffer) => {
                            ...
                            await hisoka.sendFile(from, `./temp/${sender.id}_trash.png`, `${sender.id}_trash.png`, '', id)
                ...
                try {
                    ...
                    canvas.Canvas.hitler(ypf)
                        .then(async (buffer) => {
                            ...
                            await hisoka.sendFile(from, `./temp/${sender.id}_hitler.png`, `${sender.id}_hitler.png`, '', id)
            ...
            case 'wasted':
                ...
                if (isMedia && type === 'image' || isQuotedImage) {
                    ...
                    await hisoka.reply(from, eng.wait(), id)
                    await hisoka.sendFileFromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${fotoWtNya}`, 'Wasted.jpg', 'Ini..., sticker nya lagi di kirim', id).then(() => hisoka.sendStickerfromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${fotoWtNya}`))
            ...
            case 'kiss':
                ...
                try {
                    if (isMedia && isImage || isQuotedImage) {
                        ...
                        await hisoka.reply(from, eng.wait(), id)
                        const encryptMedia = isQuotedImage ? quotedMsg : message
                        const ppRaw = await hisoka.getProfilePicFromServer(sender.id)
                        ...
                        canvas.Canvas.kiss(ppFirst, ppSecond)
                            .then(async (buffer) => {
                                ...
                                await hisoka.sendFile(from, `${sender.id}_kiss.png`, `${sender.id}_kiss.png`, '', id)
            ...
            case 'phcomment':
                ...
                const ppPhRaw = await hisoka.getProfilePicFromServer(sender.id)
                ...
                await hisoka.sendFileFromUrl(from, preprocessPh.data.message, 'ph.jpg', '', id)
            ...
            case 'neon':
                ...
                await hisoka.reply(from, eng.wait(), id)
                await hisoka.sendFileFromUrl(from, `http://docs-jojo.herokuapp.com/api/neon?text1=${atasnya}&text2=${tengahnya}&text3=${bawahnya}`, 'neon.jpg', '', id)
                console.log('Success creating image!')
            break
            case 'firemaker':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/fire_maker?text=${q}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            case 'mlmaker':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                await hisoka.reply(from, eng.wait(), id)
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/logoml?hero=${namaHero}&text=${teksMl}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            case 'balloonmaker':
            case 'blmaker':
                ...
                await hisoka.reply(from, eng.wait(), id)
                await hisoka.sendFileFromUrl(from, `https://api.vhtear.com/balloonmaker?text1=${namaKiri}&text2=${namaKanan}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            case 'sliding':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                await hisoka.sendVideoAsGif(from, `https://api.vhtear.com/slidingtext?text=${q}&apikey=${config.vhtear}`, 'sliding.gif', '', id)
            ...
            case 'stickernobg': //by: VideFrelan
                ...
                if (isMedia && type === 'image' || isQuotedImage) {
                    ...
                    misc.stickernobg(q)
                    .then(async ({ result }) => {
                        await hisoka.sendStickerfromUrl(from, result.image)
                            ...
                            sharp(res)
                                ...
                                .toFile(`./temp/stage_${sender.id}.webp`, async (err) => {
                                    ...
                                    if (fs.existsSync(`./temp/${sender.id}.webp`)) {
                                        ...
                                        await hisoka.sendRawWebpAsSticker(from, base64)
                                        console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
            ...
            case 'stikergif':
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif' || isQuotedVideo || isQuotedGif) {
                    ...
                    try {
                        ...
                        await hisoka.sendMp4AsSticker(from, videoBase64, { fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', loop: 0 })
                            .then(async () => {
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                                await hisoka.sendText(from, eng.ok())
                            })
                    } catch (err) {
                        console.error(err)
                        await hisoka.reply(from, eng.videoLimit(), id)
            ...
            case 'ttg':
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.reply(from, eng.wait(), id)
                await hisoka.sendStickerfromUrl(from, `https://api.vhtear.com/textxgif?text=${q}&apikey=${config.vhtear}`)
            ...
            case 'toimg':
                ...
                if (isQuotedSticker) {
                    ...
                    try {
                        ...
                        await hisoka.sendFile(from, imageBase64, 'sticker.jpg', '', id)
                    } catch (err) {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
                    }
            ...
            case 'emojistiker':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (args.length !== 1) return hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const emoji = emojiUnicode(args[0])
                await hisoka.reply(from, eng.wait(), id)
                console.log('Creating emoji code for =>', emoji)
                await hisoka.sendStickerfromUrl(from, `https://api.vhtear.com/emojitopng?code=${emoji}&apikey=${config.vhtear}`)
                    .then(async () => {
                        await hisoka.reply(from, eng.ok(), id)
                        console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await hisoka.reply(from, 'Emoji not supported!', id)
                    })
            break
            ...
            case 'mlewd':
                ...
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await hisoka.reply(from, eng.notNsfw(), id)
                    if (!isPremium) return await hisoka.reply(from, eng.notPremium(), id)
                    await hisoka.reply(from, eng.botNotPremium(), id)
                } else {
                    if (!isPremium) return await hisoka.reply(from, eng.notPremium(), id)
                    await hisoka.reply(from, eng.botNotPremium(), id)
                }
            ...
            case 'lewd':
                ...
                if (isGroupMsg) {
                    ...
                    nsfw.randomLewd()
                        .then(async ({ url }) => {
                            await hisoka.sendFileFromUrl(from, url, 'lewd.jpg', '', null, null, true)
            ...
            case 'fetish':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (ar.length !== 1) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await hisoka.reply(from, eng.notNsfw(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    ...
                    try {
                        if (ar[0] === 'armpits') {
                            nsfw.armpits()
                                .then(async ({ url }) => {
                                    await hisoka.sendFileFromUrl(from, url, 'armpits.jpg', '', id)
                                        .then(() => console.log('Success sending armpits pic!'))
                                })
                        } else if (ar[0] === 'feets') {
                            nsfw.feets()
                                .then(async ({ url }) => {
                                    await hisoka.sendFileFromUrl(from, url, 'feets.jpg', '', id)
                                        .then(() => console.log('Success sending feets pic!'))
                                })
                        } else if (ar[0] === 'thighs') {
                            nsfw.thighs()
                                .then(async ({ url }) => {
                                    await hisoka.sendFileFromUrl(from, url, 'thighs.jpg', '', id)
                                        .then(() => console.log('Success sending thighs pic!'))
                                })
                        } else if (ar[0] === 'ass') {
                            nsfw.ass()
                                .then(async ({ url }) => {
                                    await hisoka.sendFileFromUrl(from, url, 'ass.jpg', '', id)
                                        .then(() => console.log('Success sending ass pic!'))
                                })
                        } else if (ar[0] === 'boobs') {
                            nsfw.boobs()
                                .then(async ({ url }) => {
                                    await hisoka.sendFileFromUrl(from, url, 'boobs.jpg', '', id)
                                        .then(() => console.log('Success sending boobs pic!'))
                                })
                        } else if (ar[0] === 'belly') {
                            nsfw.belly()
                                .then(async ({ url }) => {
                                    await hisoka.sendFileFromUrl(from, url, 'belly.jpg', '', id)
                                        .then(() => console.log('Success sending belly pic!'))
                                })
                        } else if (ar[0] === 'sideboobs') {
                            nsfw.sideboobs()
                                .then(async ({ url }) => {
                                    await hisoka.sendFileFromUrl(from, url, 'sideboobs.jpg', '', id)
                                        .then(() => console.log('Success sending sideboobs pic!'))
                                })
                        } else if (ar[0] === 'ahegao') {
                            nsfw.ahegao()
                                .then(async ({ url }) => {
                                    await hisoka.sendFileFromUrl(from, url, 'ahegao.jpg', '', id)
                                        .then(() => console.log('Success sending ahegao pic!'))
                                })
                        } else {
                            await hisoka.reply(from, 'Tag not found.', id)
                        }
                    } catch (err) {
                        console.error(err)
                        await hisoka.reply(from, err, id)
                    }
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            case 'nh':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (args.length !== 1) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (isNaN(Number(args[0]))) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await hisoka.reply(from, eng.notNsfw(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    ...
                    if (validate === true) {
                        try {
                            ...
                            await hisoka.sendFileFromUrl(from, pic, 'nhentai.jpg', teks, id)
                        ...
                        await hisoka.reply(from, eng.nhFalse(), id)
                    }
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                ...
                if (isGroupMsg) {
                    ...
                    nana.search(q)
                        .then(async (g) => {
                            ...
                            await hisoka.sendFileFromUrl(from, g.results[0].thumbnail.s, `${g.results[0].title}`, txt, id)
            ...
            case 'nekopoi':
                ...
                if (isGroupMsg) {
                    ...
                    try {
                        ...
                        await hisoka.reply(from, text, id)
                    } catch (err) {
                        console.error(err)
                        await hisoka.reply(from, 'Error!', id)
                    }
            ...
            case 'phdl':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isUrl(url) && !url.includes('pornhub.com')) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await hisoka.reply(from, eng.notNsfw(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            case 'yuri':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.reply(from, eng.wait(), id)
                    await hisoka.sendFileFromUrl(from, (await neko.nsfw.eroYuri()).url, 'yuri.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.reply(from, eng.wait(), id)
            ...
            case 'lewdavatar':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.reply(from, eng.wait(), id)
                    await hisoka.sendFileFromUrl(from, (await neko.nsfw.avatar()).url, 'avatar.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.reply(from, eng.wait(), id)
            ...
            case 'femdom':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.reply(from, eng.wait(), id)
                    await hisoka.sendFileFromUrl(from, (await neko.nsfw.femdom()).url, 'femdom.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.reply(from, eng.wait(), id)
            ...
            case 'cersex':
                ...
                if (isGroupMsg) {
                    ...
                    nsfw.cersex()
                        .then(async ({ result }) => {
                            await hisoka.sendFileFromUrl(from, result.image, 'cersex.jpg', `── *「 ${result.judul} 」* ──\n\n${result.cerita}`, id)
            ...
            case 'revoke':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isGroupMsg) return await hisoka.reply(from, eng.groupOnly(), id)
                if (!isGroupAdmins) return hisoka.reply(from, eng.adminOnly(), id)
                if (!isBotGroupAdmins) return hisoka.reply(from, eng.botNotAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.revokeGroupInviteLink(groupId);
                hisoka.sendTextWithMentions(from, `Group link revoked by @${sender.id.replace('@c.us', '')}`)
            break
            case 'linkgroup':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isGroupMsg) return await hisoka.reply(from, eng.groupOnly(), id)
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                if (!isBotGroupAdmins) return await hisoka.reply(from, eng.botNotAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const gcLink = await hisoka.getGroupInviteLink(groupId)
                const linkGc = `Group: *${formattedTitle}*\n\nLink: ${gcLink}`
                hisoka.reply(from, linkGc, id)
            break
            case 'ownergroup':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isGroupMsg) return await hisoka.reply(from, eng.groupOnly(), id)
                ...
                await hisoka.sendTextWithMentions(from, `Owner Group : @${ownerGc}`)
            break
            case 'mutegc':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isGroupMsg) return hisoka.reply(from, eng.groupOnly(), id)
                ...
                if (ar[0] === 'enable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.setGroupToAdminsOnly(groupId, true)
                    await hisoka.sendText(from, eng.gcMute())
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.setGroupToAdminsOnly(groupId, false)
                    await hisoka.sendText(from, eng.gcUnmute())
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'add':
                ...
                try {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await hisoka.addParticipant(from, `${args[0]}@c.us`)
                    await hisoka.sendText(from, '🎉 Welcome! 🎉')
                } catch (err) {
                    console.error(err)
                    await hisoka.reply(from, 'Error!', id)
                }
            ...
            case 'kick':
                ...
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                if (!isBotGroupAdmins) return await hisoka.reply(from, eng.botNotAdmin(), id)
                if (mentionedJidList.length === 0) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.sendTextWithMentions(from, `Good bye~\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
                for (let i of mentionedJidList) {
                    if (groupAdmins.includes(i)) return await hisoka.sendText(from, eng.wrongFormat())
                    await hisoka.removeParticipant(groupId, i)
                }
            break
            case 'promote':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                if (!isBotGroupAdmins) return await hisoka.reply(from, eng.botNotAdmin(), id)
                if (mentionedJidList.length !== 1) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (groupAdmins.includes(mentionedJidList[0])) return await hisoka.reply(from, eng.adminAlready(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.promoteParticipant(groupId, mentionedJidList[0])
                await hisoka.reply(from, eng.ok(), id)
            break
            case 'demote':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                if (mentionedJidList.length !== 1) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (!groupAdmins.includes(mentionedJidList[0])) return await hisoka.reply(from, eng.notAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await hisoka.demoteParticipant(groupId, mentionedJidList[0])
                await hisoka.reply(from, eng.ok(), id)
            break
            case 'leave':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                if (!isGroupMsg) return await hisoka.reply(from, eng.groupOnly(), id)
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                await hisoka.sendText(from, 'Sayounara~ 👋')
                await hisoka.leaveGroup(groupId)
            break
            case 'admins':
            case 'admin':
                if (!isRegistered) return await hisoka.reply(from, eng.notRegistered(), id)
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const groupAdm = await hisoka.getGroupAdmins(groupId)
                ...
                if (lastAdmin !== undefined && cd - (Date.now() - lastAdmin) > 0) {
                    ...
                    await hisoka.sendTextWithMentions(from, txt)
                } else {
                    let txt = '╔══✪〘 *ADMINS* 〙✪══\n'
                    for (let i = 0; i < groupAdm.length; i++) {
                        txt += '╠➥'
            ...
            case 'everyone':
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const groupMem = await hisoka.getGroupMembers(groupId)
            ...
            case 'groupicon':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    const mediaData = await decryptMedia(encryptMedia, uaOverride)
                    const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
                    await hisoka.setGroupIcon(groupId, imageBase64)
                    await hisoka.sendText(from, eng.ok())
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            ...
            case 'antilink':
                ...
                if (!isBotGroupAdmins) return await hisoka.reply(from, eng.botNotAdmin(), id)
                if (ar[0] === 'enable') {
                    if (isDetectorOn) return await hisoka.reply(from, eng.detectorOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antilink.push(groupId)
                    fs.writeFileSync('./database/group/antilink.json', JSON.stringify(_antilink))
                    await hisoka.reply(from, eng.detectorOn(name, formattedTitle), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antilink.splice(groupId, 1)
                    fs.writeFileSync('./database/group/antilink.json', JSON.stringify(_antilink))
                    await hisoka.reply(from, eng.detectorOff(), id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'leveling':
                ...
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isLevelingOn) return await hisoka.reply(from, eng.levelingOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _leveling.push(groupId)
                    fs.writeFileSync('./database/group/leveling.json', JSON.stringify(_leveling))
                    await hisoka.reply(from, eng.levelingOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _leveling.splice(groupId, 1)
                    fs.writeFileSync('./database/group/leveling.json', JSON.stringify(_leveling))
                    await hisoka.reply(from, eng.levelingOff(), id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'welcome':
                ...
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isWelcomeOn) return await hisoka.reply(from, eng.welcomeOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _welcome.push(groupId)
                    fs.writeFileSync('./database/group/welcome.json', JSON.stringify(_welcome))
                    await hisoka.reply(from, eng.welcomeOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _welcome.splice(groupId, 1)
                    fs.writeFileSync('./database/group/welcome.json', JSON.stringify(_welcome))
                    await hisoka.reply(from, eng.welcomeOff(), id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            ...
            case 'autostik':
                ...
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isAutoStickerOn) return await hisoka.reply(from, eng.autoStikOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _autosticker.push(groupId)
                    fs.writeFileSync('./database/group/autosticker.json', JSON.stringify(_autosticker))
                    await hisoka.reply(from, eng.autoStikOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _autosticker.splice(groupId, 1)
                    fs.writeFileSync('./database/group/autosticker.json', JSON.stringify(_autosticker))
                    await hisoka.reply(from, eng.autoStikOff(), id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'antinsfw':
                ...
                if (!isBotGroupAdmins) return await hisoka.reply(from, eng.botNotAdmin(), id)
                if (ar[0] === 'enable') {
                    if (isDetectorOn) return await hisoka.reply(from, eng.antiNsfwOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antinsfw.push(groupId)
                    fs.writeFileSync('./database/group/antinsfw.json', JSON.stringify(_antinsfw))
                    await hisoka.reply(from, eng.antiNsfwOn(name, formattedTitle), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await hisoka.reply(from, eng.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antinsfw.splice(groupId, 1)
                    fs.writeFileSync('./database/group/antinsfw.json', JSON.stringify(_antinsfw))
                    await hisoka.reply(from, eng.antiNsfwOff(), id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            ...
            // Owner command
            case 'bc':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                if (!q) return await hisoka.reply(from, eng.emptyMess(), id)
                const chats = await hisoka.getAllChatIds()
                for (let bcs of chats) {
                    let cvk = await hisoka.getChatById(bcs)
                    if (!cvk.isReadOnly) await hisoka.sendText(bcs, `${q}\n\n- Slavyan (Kal)\n_Broadcasted message_`)
                }
                await hisoka.reply(from, eng.doneOwner(), id)
            break
            case 'clearall':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                const allChats = await hisoka.getAllChats()
                for (let delChats of allChats) {
                    await hisoka.deleteChat(delChats.id)
                }
                await hisoka.reply(from, eng.doneOwner(), id)
            break
            case 'leaveall':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                if (!q) return await hisoka.reply(from, eng.emptyMess(), id)
                const allGroup = await hisoka.getAllGroups()
                for (let gclist of allGroup) {
                    await hisoka.sendText(gclist.contact.id, q)
                    await hisoka.leaveGroup(gclist.contact.id)
                }
                await hisoka.reply(from, eng.doneOwner())
            break
            case 'getses':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                const ses = await hisoka.getSnapshot()
                await hisoka.sendFile(from, ses, 'session.png', eng.doneOwner())
            break
            case 'ban':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                if (ar[0] === 'add') {
                    if (mentionedJidList.length !== 0) {
                        for (let benet of mentionedJidList) {
                            if (benet === botNumber) return await hisoka.reply(from, eng.wrongFormat(), id)
                            _ban.push(benet)
            ...
            case 'ev':
                ...
                try {
                    ...
                    await hisoka.sendText(from, evaled)
                } catch (err) {
                    console.error(err)
                    await hisoka.reply(from, 'Error!', id)
                }
            ...
            case 'shutdown':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                await hisoka.sendText(from, 'Otsukaresama deshita~ 👋')
                    .then(async () => await hisoka.kill())
                ...
                if (ar[0] === 'add') {
                    if (mentionedJidList.length !== 0) {
                        for (let prem of mentionedJidList) {
                            if (prem === botNumber) return await hisoka.reply(from, eng.wrongFormat(), id)
            ...
            case 'mute':
                ...
                if (!isGroupAdmins) return await hisoka.reply(from, eng.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isMute) return await hisoka.reply(from, eng.muteChatOnAlready(), id)
                    _mute.push(groupId)
                    fs.writeFileSync('./database/bot/mute.json', JSON.stringify(_mute))
                    await hisoka.reply(from, eng.muteChatOn(), id)
                } else if (ar[0] === 'disable') {
                    _mute.splice(groupId, 1)
                    fs.writeFileSync('./database/bot/mute.json', JSON.stringify(_mute))
                    await hisoka.reply(from, eng.muteChatOff(), id)
                } else {
                    await hisoka.reply(from, eng.wrongFormat(), id)
                }
            break
            case 'setname':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                if (!q || q.length > 25) return await hisoka.reply(from, eng.wrongFormat(), id)
                await hisoka.setMyName(q)
                await hisoka.reply(from, `Done!\n\nUsername changed to: ${q}`, id)
            break
            case 'give':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                if (args.length !== 2) return await hisoka.reply(from, eng.wrongFormat(), id)
                if (mentionedJidList.length !== 0) {
                    for (let give of mentionedJidList) {
                        ...
                        await hisoka.reply(from, `Sukses menambah XP kepada: ${give}\nJumlah ditambahkan: ${args[1]}`, id)
                    }
                } else {
                    level.addLevelingXp(args[0] + '@c.us', Number(args[1]), _level)
                    await hisoka.reply(from, `Sukses menambah XP kepada: ${args[0]}\nJumlah ditambahkan: ${args[1]}`, id)
                }
            break
            case 'listgroup':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
                    hisoka.getAllGroups().then((res) => {
                    let gc = '*Group list*:\n'
                    for (let i = 0; i < res.length; i++) {
                        gc += `\n\n*No*: ${i+1}\n*Nama*: ${res[i].name}\n*Unread messages*: ${res[i].unreadCount} messages\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                    }
                    hisoka.reply(from, gc, id)
                })
            break
            case 'reset':
                if (!isOwner) return await hisoka.reply(from, eng.ownerOnly(), id)
            ...
            default:
                if (isCmd) {
                    await hisoka.reply(from, eng.cmdNotFound(command), id)
                }
            break
        }
    } catch (err) {
